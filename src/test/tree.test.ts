...
import { ListToTreeOps } from "../utils/type";
...
describe("TreeUtils", () => {
  describe("initTree", () => {
    it("should initialize a tree structure correctly", () => {
      ...
      expect(result[0].children[0].level).toBe(TreeLevel.Two);
      expect(result[0].children[0].expanded).toBe(true);
      expect(result[0].children[0].uniKey).toBe("0-0");
      expect(result[1].level).toBe(TreeLevel.One);
      expect(result[1].expanded).toBe(true);
      expect(result[1].children.length).toBe(1);
      expect(result[1].children[0].level).toBe(TreeLevel.Two);
      expect(result[1].children[0].expanded).toBe(true);
      expect(result[1].children[0].uniKey).toBe("1-0");

      //Check lastArray
      expect(result[0].lastArray).toEqual([IsLast.F]);
      expect(result[1].lastArray).toEqual([IsLast.T]);
      expect(result[0].children[0].lastArray).toEqual([IsLast.F, IsLast.F]);
      expect(result[0].children[1].lastArray).toEqual([IsLast.F, IsLast.T]);
      expect(result[1].children[0].lastArray).toEqual([IsLast.T, IsLast.T]);

      //Check idxs
      expect(result[0].idxs).toEqual([0]);
      expect(result[1].idxs).toEqual([1]);
      expect(result[0].children[0].idxs).toEqual([0, 0]);
      expect(result[0].children[1].idxs).toEqual([0, 1]);
      expect(result[1].children[0].idxs).toEqual([1, 0]);
    });
